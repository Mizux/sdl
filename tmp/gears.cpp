...
static struct gear* create_gear(
  ...
  for (int i = 0; i < teeth; i++) {
    ...
    sincos(i * 2.0f * float{M_PI} / teeth, &s[0], &c[0]);
    sincos(i * 2.0f * float{M_PI} / teeth + da, &s[1], &c[1]);
    sincos(i * 2.0f * float{M_PI} / teeth + da * 2, &s[2], &c[2]);
    sincos(i * 2.0f * float{M_PI} / teeth + da * 3, &s[3], &c[3]);
    sincos(i * 2.0f * float{M_PI} / teeth + da * 4, &s[4], &c[4]);
...
#define GEAR_VERT(v, point, sign) \
  vert((v), p[(point)].x, p[(point)].y, (sign) * width * 0.5f, normal)
...
#define QUAD_WITH_NORMAL(p1, p2)                                      \
  ...
  glBufferData(
      GL_ARRAY_BUFFER, gear->nvertices * sizeof(GearVertex), gear->vertices, GL_STATIC_DRAW);
...
void perspective(GLfloat* m, GLfloat fovy, GLfloat aspect, GLfloat zNear, GLfloat zFar) {
  ...
  GLfloat radians = fovy / 2.0f * GLfloat{M_PI} / 180.0f;
...
static void draw_gear(
  ...
  rotate(model_view, 2 * M_PI * angle / 360.0f, 0, 0, 1);
...
static void gears_draw(void) {
  ...
  rotate(transform, 2 * M_PI * view_rot[0] / 360.0f, 1, 0, 0);
  rotate(transform, 2 * M_PI * view_rot[1] / 360.0f, 0, 1, 0);
  rotate(transform, 2 * M_PI * view_rot[2] / 360.0f, 0, 0, 1);
...
static void gears_reshape(int width, int height) {
  ...
  perspective(ProjectionMatrix, 60.0f, width / (float)height, 1.0f, 1024.0f);
...
static void gears_event(SDL_Event* event) {
  switch (event->type) {
    case SDL_KEYDOWN: {
...
static void gears_idle(void) {
  ...
  double        dt, t                = SDL_GetTicks() / 1000.0f;
